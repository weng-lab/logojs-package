...
const _position =
  (width: number, height: number) => (lv: number[], transform: string, key: number, alphabet: any, events: any) => {
    ...
    return (
      <GlyphStack
        indices={indices}
        alphabet={alphabet}
        onSymbolMouseOver={onSymbolMouseOver ? (s: string) => onSymbolMouseOver(key, s) : undefined}
        onSymbolClick={onSymbolClick ? (s: string) => onSymbolClick(key, s) : undefined}
        onSymbolMouseOut={onSymbolMouseOut ? (s: string) => onSymbolMouseOut(key, s) : undefined}
        lv={lv}
        transform={transform}
        width={width}
        height={height}
...
export interface RawLogoProps {
  values: number[][];
  glyphWidth: number;
  stackHeight: number;
  alphabet: Symbol[];
  onSymbolMouseOver: ((symbol: Symbol) => any) | undefined;
  onSymbolMouseOut: ((symbol: Symbol) => any) | undefined;
  onSymbolClick: ((symbol: Symbol) => any) | undefined;
}
...
export interface LogoProps {
  ...
  values?: number[][];
  fasta?: string;
  mode?: "FREQUENCY" | "INFORMATION_CONTENT";
  height?: number;
  width?: number;
  alphabet: Symbol[];
  glyphwidth?: number;
  ...
  yAxisMax?: number;
  onSymbolMouseOver?: (symbol: Symbol) => any;
  onSymbolMouseOut?: (symbol: Symbol) => any;
  onSymbolClick?: (symbol: Symbol) => any;
...
const Logo = React.forwardRef(
  (
    ...
    const relativePseudocount =
      (pfm || fasta) && !constantPseudocount && !countUnaligned ? !smallSampleCorrectionOff : false;
    ...
    const sums =
      ...
      pfm
        ...
        .map((x) => (x === 0 ? 0 : (alphabet.length - 1) / (2 * Math.log(2) * x)));
    if (!ppm && pfm && pfm.map)
      ppm = pfm.map((column) => {
        const sum =
          (count && countUnaligned ? count : column.reduce((a, c) => a + c, 0.0) + pseudocount * alphabet.length) || 1;
        return column.map((x) => (x + pseudocount) / sum);
      });
    if (!ppm || ppm.length === 0 || ppm[0].length === 0) return <div />;
    let alphabetSize = ppm[0].length;
    if (!backgroundFrequencies) backgroundFrequencies = ppm[0].map((_) => 1.0 / alphabetSize);
    let likelihood =
      values ||
      (mode === INFORMATION_CONTENT && sums
        ? ppm.map((x, i) => logLikelihood(backgroundFrequencies)(x, sums[i]))
        : ppm.map((x) => x.map((v) => v * Math.log2(alphabetSize))));
    const theights =
      mode === FREQUENCY ? [Math.log2(alphabetSize)] : backgroundFrequencies.map((x) => Math.log2(1.0 / (x || 0.01)));
    const max = yAxisMax || Math.max(...theights),
      min = Math.min(...theights);
    const zeroPoint = min < 0 ? max / (max - min) : 1.0;

    /* misc options */
    startpos = startpos && !isNaN(startpos) && isFinite(startpos) ? startpos : 1;
