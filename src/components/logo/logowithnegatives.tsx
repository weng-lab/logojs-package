...
const _position =
  ...
  (lv: number[], transform: string, key: number, alphabet: any, negative: boolean, events: any) => {
    ...
    return (
      <GlyphStack
        ...
        width={width}
        height={height}
        onSymbolMouseOver={onSymbolMouseOver ? (s) => onSymbolMouseOver(key, s) : undefined}
        onSymbolClick={onSymbolClick ? (s) => onSymbolClick(key, s) : undefined}
        onSymbolMouseOut={onSymbolMouseOut ? (s) => onSymbolMouseOut(key, s) : undefined}
        key={key}
        inverted={inverted}
      />
    );
...
export interface LogoWithNegativesProps {
  values: number[][];
  height: number;
  width: number;
  alphabet: Alphabet;
