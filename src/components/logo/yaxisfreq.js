...
class YAxisFrequency extends React.Component {

    render() {
	let ticks = xrange(this.props.ticks + 1).map(x => x / this.props.ticks);
	return (
	    <g transform={this.props.transform}>
		<rect height={this.props.height} width={4} x={this.props.width + 1} y="0"
	          fill="#000000" />
		{ticks.map( x => (
		  <g key={x} transform={"translate(0," + (this.props.height - Math.floor(x * this.props.height)) + ")"}>
